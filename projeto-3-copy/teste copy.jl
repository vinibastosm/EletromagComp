...
epsolon_0 = 1/(4*pi)
epsolon_r = 1
s = 2 #(dt*sigma)/(epsolon_r*epsolon_0)
courant = c*dt/dx
...
for n in 1:n_iter
    ...
    global E_x[:, 2:end-1] = - (s+1)/(s-1)*E_x[:, 2:end-1] + (courant/epsolon_r)*(1/(s+1)) * (B_z[:, 2:end] - B_z[:, 1:end-1])
    global E_y[2:end-1, :] = - (s+1)/(s-1)*E_y[2:end-1, :] - (courant/epsolon_r)*(1/(s+1))* (B_z[2:end, :] - B_z[1:end-1, :])
    ...
    heatmap(E_x, title="Gráfico de Calor de E_x, Iteração: $n", xlabel="X", ylabel="Y", color=:viridis)
