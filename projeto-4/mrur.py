...
def xpos(t):
    xpos = initial_speed*t
    xpos[t > 0] = initial_speed*t[t > 0] - 0.5*deceleration*t[t > 0]**2
    xpos[t > stop_t] = initial_speed * stop_t - 0.5*deceleration*stop_t**2
    return xpos
...
def xvel(t):
    xvel = initial_speed*np.ones(t.shape)
    xvel[t > 0] = initial_speed - deceleration*t[t > 0]
    xvel[t > stop_t] = 0
    return xvel
...
# Criando o array de valores de i
i_values = np.arange(aux)

# Aplicando a expressão para cada valor de i
result_array = (i_values * stop_t) / aux

pos = xpos(result_array)
...
for i in range(aux):
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(
        np.sqrt((E_total[i][0].T)**2+(E_total[i][2].T)**2),
        cmap='coolwarm',
        origin='lower',
        aspect='equal',
